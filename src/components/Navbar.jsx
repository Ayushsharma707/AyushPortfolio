...
const Navbar = () => {
        ...
        <div className="flex justify-between items-center py-2 md:py-4 md:px-2 pl-2 mx-auto">
          ...
          <div className="flex items-center cursor-pointer">
            <a href="/" className="flex items-center group relative">
              {/* Name with Animation */}
              <span
                className={`text-3xl font-extrabold ml-3 tracking-wide relative ${darkMode ? "text-black" : "text-white"
                  }`}
              >
                Ayush
                <span
                  className={`absolute left-0 bottom-0 h-1 w-0 transition-all duration-700 ${darkMode ? "bg-blue-500" : "bg-blue-500"
                    } group-hover:w-full`}
                ></span>
              </span>
              <span
                className={`text-3xl font-light ml-1 tracking-wide ${darkMode ? "text-gray-700" : "text-gray-300"
                  }`}
              >
                Sharma
              </span>
              ...
              <div
                className="absolute -top-3 -right-4 opacity-0 group-hover:opacity-100 group-hover:animate-pulse"
                style={{ pointerEvents: "none" }}
              >
                <span
                  className={`block w-2 h-2 rounded-full ${darkMode ? "bg-blue-300" : "bg-blue-300"
                    }`}
                ></span>
                <span
                  className={`block w-3 h-3 rounded-full ${darkMode ? "bg-blue-200" : "bg-blue-500"
                    } ml-2`}
                ></span>
                <span
                  className={`block w-4 h-4 rounded-full ${darkMode ? "bg-blue-100" : "bg-blue-500"
                    } ml-4`}
                ></span>
              </div>
            </a>
  ...
  `}
          ...
          <div className="hidden justify-between items-center w-full md:flex md:w-auto ">
            ...
            >
              {links.map((el) => (
                <li className="cursor-pointer" key={el.route}> {/* Use el.route or another unique value */}
                  <Link
                    to={el.route}
